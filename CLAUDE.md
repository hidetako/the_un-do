...
- **挙動**：坐禅の中断（やめる）→**入口(setup)に戻る**（splashではない）。線香は全座処**右下固定**（choの香炉配置は撤回済み＝cover背景に固定座標は破綻する）。煙は濃いめ（幹芯0.28）＋**色は sel.scene の明暗に毎フレーム追従**（暗=honden/tsuki/cho→白煙、明→ねずみ色）。**線香のディテール**（`drawIncense`・`#smoke` キャンバス／未燃部だけ DOM `.stick` の茶色）：**火点＝丸ではなく縦長（太さ:長さ＝1:2.2）の明度を落としたほんのり赤み**（微かな明滅）。火点は下方へ燃え進み、**燃えた分は火点の上に白寄りの灰色の灰＝線香形で残る**（灰は**明度控えめ**・最大長は**元長の1/8**）。**灰の落下＝ヒンジモデル**：最大長に達すると（またはときどき手前で）**根元を火点にピン留めしたまま重力トルクで倒れ込み（跳ね上がらない）、60°を超えたら根元が離れて“そのまま真下に”自由落下**（解放時は横速度を弱め・回転もゆるめる）→**皿＝キャンバス下端まで落ちてから淵に隠れて消える**（フェードで途中消滅させない）。実装は `ashBits`（ph0=ヒンジ／ph1=自由落下）。**煙は火点の最上部から**（灰が煙の根元を隠し、煙は灰の上から立つ＝煙を描いた後に火点・灰を重ねる）。`.stick` に height の transition は付けない（火点との隙間防止）。配信の周期／再開で線香が伸びたら灰リセット。`prefers-reduced-motion` で明滅・落下は無効（灰は静かに1/8で頭打ち）。
