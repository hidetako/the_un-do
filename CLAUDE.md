...
- **挙動**：坐禅の中断（やめる）→**入口(setup)に戻る**（splashではない）。線香は全座処**右下固定**（choの香炉配置は撤回済み＝cover背景に固定座標は破綻する）。煙は濃いめ（幹芯0.28）＋**色は sel.scene の明暗に毎フレーム追従**（暗=honden/tsuki/cho→白煙、明→ねずみ色）。**線香のディテール**（`drawIncense`・`#smoke` キャンバス／未燃部だけ DOM `.stick` の茶色）：**火点＝丸ではなく縦長（太さ:長さ＝1:2.2）の明度を落としたほんのり赤み**（微かな明滅）。火点は下方へ燃え進み、**燃えた分は火点の上に白寄りの灰色の灰＝線香形で残る**（灰は**明度控えめ**・最大長は**元長の1/8**）。**灰の落下＝ヒンジモデル**：最大長に達すると（またはときどき手前で）**根元を火点にピン留めしたまま重力トルクで倒れ込み（跳ね上がらない・倒れ始めはトルクを EASE=0.5秒 かけて立ち上げ＝ふわっと）、60°を超えたら根元が離れて“そのまま真下に”自由落下**（解放時は横速度を弱め・回転もゆるめる）→**皿＝キャンバス下端まで落ちてから淵に隠れて消える**（フェードで途中消滅させない）。実装は `ashBits`（ph0=ヒンジ／ph1=自由落下）。**煙は火点の最上部から**（灰が煙の根元を隠し、煙は灰の上から立つ＝煙を描いた後に火点・灰を重ねる）。`.stick` に height の transition は付けない（火点との隙間防止）。配信の周期／再開で線香が伸びたら灰リセット。`prefers-reduced-motion` で明滅・落下は無効（灰は静かに1/8で頭打ち）。
...
- **坐禅セッション体験**：Wake Lock で坐禅中は眠らせない。ただし **iPad/Chrome for iOS は `navigator.wakeLock` を出さず自動ロック→スリープでき、その間 `setTimeout` も凍る**（＝45分後の終了鐘が来ない事故）。対策＝終了予定を**実時刻 `endAt` でも保持**し、**復帰（`visibilitychange`/`pageshow`）した瞬間に精算**する `onResume`／`finishSit`：過ぎていればその場で鐘＋合掌、まだなら残り時間でタイマー引き直し＋`ac.resume()`。線香開始90秒で**墨色の暗幕**（opacity .94・タップで復帰・背景だけ沈み線香は残る＝仕様）。2分以上の坐は開始時にアナウンス（9言語）＝**Wake Lock が使える端末は `dim_note`「暗くなるが眠らない」**、使えない端末（Chrome for iOS 等 `'wakeLock' in navigator` が false）は**正直な `dim_sleep`「暗くなる／スリープしても戻れば鳴る」＋未インストールなら `install_tip`（ホーム画面追加を勧める）**。「ただ、座る」は**30秒でフェードアウト**。**終了＝鐘→暗転解除→「合掌」**（ja/中文=合掌・韓=합장・欧文=合掌 · gasshō）**が5.6秒浮かんで end 画面へ**。終了トリガーは実時間 `endTimer`（rAF非依存＝裏タブ/ロックでも時間どおり）。
